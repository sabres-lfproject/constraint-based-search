...
vector<Path> CBS_Solver::find_solution(SubstrateGraph& SG, VNRequest& aRequest, double w) {
  ...
  for (int i = num_vertices; i < aRequest.nodeNum + num_vertices; i++) {
    ...
    int validNodeCount = myFindNodesWithinConstraints(
        aRequest.nodes[i - num_vertices], aRequest.reqID, aRequest.maxD, SG_copy, metaNodes[i]);
  ...
  for (int i = 0; i < aRequest.edgeNum; i++) {
    requirement_list.push_back(
        make_tuple(aRequest.edges[i].from + num_vertices, aRequest.edges[i].to + num_vertices, aRequest.edges[i].bw));
  }
  ...
  for (auto req : requirement_list) {
    Path p = (find_requirement_path(
        req, SG_copy, metaNodes, metaEdges, root->constraint_list[req_count], root->paths, req_count, true, aRequest));
    if (p.empty()) {
  ...
  while (!FOCAL.empty()) {
    ...
    if (cons_v2.empty()) {
      for (Constraint cons : cons_v) {
        ...
        if (find(child->constraint_list[path_to_update].begin(),
                 child->constraint_list[path_to_update].end(),
                 make_pair(get<1>(cons), get<2>(cons))) == child->constraint_list[path_to_update].end()) {
        ...
        Path new_path = find_requirement_path(requirement_list[path_to_update],
                                              ...
                                              metaNodes,
                                              metaEdges,
                                              child->constraint_list[path_to_update],
                                              child->paths,
                                              path_to_update,
                                              false,
                                              aRequest);
...
Path CBS_Solver::find_requirement_path(std::tuple<int, int, double>& requirement,
                                       SubstrateGraph& SN,
                                       map<int, vector<int>>& metaNodes,
                                       map<int, vector<int>>& metaEdges,
...
int CBS_Solver::myFindNodesWithinConstraints(Node& aNode,
                                             int reqID,
                                             int maxD,
                                             SubstrateGraph& SN,
  ...
  for (i = 0; i < SN.nodeNum; i++) {
    if ((SN.nodes[i].distanceFrom(aNode) <= maxD) && (SN.nodes[i].rest_cpu >= aNode.cpu) &&
        (SN.nodes[i].touched == false)) {
...
vector<int> CBS_Solver::getNeighbors(int nodeID,
                                     map<int, vector<int>> metaNodes,
                                     map<int, vector<int>> metaEdges,
                                     SubstrateGraph& SN) {
...
Conflict CBS_Solver::check_conflicts(CBS_Node* n,
                                     vector<Path>& paths,
                                     VNRequest& aRequest,
                                     vector<std::tuple<int, int, double>>& requirement_list,
                                     SubstrateGraph& SN) {
  ...
  for (Path p : paths) {
    ...
    if (it == VN_SN_one_to_one.end()) {  // this vn hasn't been mapped yet.
      ...
      if (p[1] != VN_SN_one_to_one[p[0]]) {
        Conflict c =
            make_tuple(2, path_id, paths_used_node[VN_SN_one_to_one[p[0]]], p[0], p[1], p[0], VN_SN_one_to_one[p[0]]);
        return c;
      } else {
        paths_used_node[p[1]].push_back(path_id);
      }
    ...
    if (it2 == VN_SN_one_to_one.end()) {
      ...
      if (it_sn_vn2 != SN_VN_one_to_one.end()) {
        Conflict c = make_tuple(2,
                                path_id,
                                paths_used_node[p[last_sn_idx]],
                                p[end_idx],
                                p[last_sn_idx],
                                SN_VN_one_to_one[p[last_sn_idx]],
                                p[last_sn_idx]);
        return c;
      } else {
      ...
      if (p[last_sn_idx] != VN_SN_one_to_one[p[end_idx]]) {
        Conflict c = make_tuple(2,
                                path_id,
                                paths_used_node[VN_SN_one_to_one[p[end_idx]]],
                                p[end_idx],
                                p[last_sn_idx],
                                p[end_idx],
                                VN_SN_one_to_one[p[end_idx]]);
        return c;
      } else {
        paths_used_node[p[last_sn_idx]].push_back(path_id);
      }
...
int CBS_Solver::count_num_conflicts(CBS_Node* n,
                                    vector<Path>& paths,
                                    VNRequest& aRequest,
                                    vector<std::tuple<int, int, double>>& requirement_list,
                                    SubstrateGraph& SN) {
...
double CBS_Solver::calculate_mapping_cost(vector<Path> paths,
                                          VNRequest& vnr,
                                          SubstrateGraph& SG,
                                          vector<std::tuple<int, int, double>>& requirement_list) {
  double cost_sum = 0.0;
