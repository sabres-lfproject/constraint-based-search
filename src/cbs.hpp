...
struct Compare_Path_Usage {
  bool operator()(const pair<int, double>& p1, const pair<int, double>& p2) const { return p1.second <= p2.second; }
};
...
class CBS_Node {
 public:
  ...
  struct compare_node_focal {
    bool operator()(const CBS_Node* n1, const CBS_Node* n2) const { return n1->num_conflict >= n2->num_conflict; }
  };
 ...
 public:
  ...
  struct compare_node {
    bool operator()(const A_Node* n1, const A_Node* n2) const {
      if (n1->cost == n2->cost) {
        return n1->times_used_by_other_paths >=
               n2->times_used_by_other_paths;  // prefer the one that has been used fewer times by other vns.
      }
      return n1->cost >= n2->cost;
    }
  };
...
class CBS_Solver {
 public:
  ...
  Path find_requirement_path(std::tuple<int, int, double>& requirement,
                             SubstrateGraph& SN,
  ...
  int myFindNodesWithinConstraints(Node& aNode, int reqID, int maxD, SubstrateGraph& SN, vector<int>& validNodeIDs);
  void update_VNR_info(vector<Path> paths, VNRequest& vnr, SubstrateGraph& SG);
  double calculate_mapping_cost(vector<Path> paths,
                                VNRequest& vnr,
                                SubstrateGraph& SG,
 ...
 private:
  vector<int> getNeighbors(int nodeID,
                           ...
                           map<int, vector<int>> metaEdges,
                           SubstrateGraph& SN);
  Conflict check_conflicts(CBS_Node* n,
                           vector<Path>& paths,
                           VNRequest& aRequest,
                           vector<std::tuple<int, int, double>>& requirement_list,
                           SubstrateGraph& SN);
  int count_num_conflicts(CBS_Node* n,
                          vector<Path>& paths,
                          VNRequest& aRequest,
                          vector<std::tuple<int, int, double>>& requirement_list,
                          SubstrateGraph& SN);
